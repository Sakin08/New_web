...
const NavLink = ({ to, text, Icon }) => (
  <Link
    to={to}
    className="flex items-center space-x-1.5 px-3 py-2 text-gray-700 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition font-medium text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
  >
...
const DropdownMenu = ({ title, items, isOpen, onToggle }) => {
  return (
    <div className="relative">
      <button
        onClick={onToggle}
        className="flex items-center space-x-1 px-3 py-2 text-gray-700 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition font-medium text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
      >
        <span>{title}</span>
      ...
      {isOpen && (
        <>
          <div
            ...
            onClick={onToggle}
          ></div>
          <div className="absolute left-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 z-20 transform origin-top-left animate-fade-in">
            {items.map((item, index) => (
              <Link
                key={index}
                to={item.to}
                onClick={onToggle}
                className="flex items-center space-x-3 px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition"
              >
...
const MobileNavLink = ({ to, Icon, text, onClick }) => (
  <Link
    to={to}
    onClick={onClick}
    className="flex items-center space-x-4 px-4 py-3 text-gray-700 hover:bg-gray-100 rounded-xl transition font-medium"
  >
...
const Navbar = () => {
  ...
  const menuItems = isOtherRole ? {
    ...
    community: {
      title: "Community",
      items: [
        { to: '/study-groups', Icon: BookOpen, text: 'Study Groups' },
    ...
    <nav className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-lg">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link
            to="/"
            className="flex items-center space-x-2 group focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-md"
            title="SUST Connect"
          >
            <img
              src="/image/482984952_993190959578541_8366529342364279980_n.jpg"
              alt="SUST Logo"
              className="w-10 h-10 rounded-xl object-cover transform group-hover:scale-105 transition-transform shadow-lg ring-2 ring-indigo-100"
            />
            <span className="text-xl font-extrabold">
              <span className="bg-gradient-to-r from-red-600 to-red-700 bg-clip-text text-transparent">SUST</span>
          ...
          <div className="flex items-center space-x-3">
              ...
              <>
                ...
                <div className="relative hidden lg:block">
                  <button
                    ...
                    className="flex items-center space-x-2 p-1.5 bg-gray-50 hover:bg-gray-100 rounded-full transition border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                  >
                    {user.profilePicture ? (
                      <img
                      ...
                      <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                        {user.name?.charAt(0).toUpperCase()}
                      </div>
                    )}
                    <span className="text-sm font-semibold text-gray-800 hidden xl:inline max-w-[120px] truncate">
                      {user.name}
                    </span>
                    <ChevronDown className={`w-4 h-4 text-gray-500 transition-transform mr-1 ${showUserMenu ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  {showUserMenu && (
                    <>
                      ...
                      ></div>
                      <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 z-20 transform origin-top-right animate-fade-in">
                        ...
                        {user.role === 'admin' && (
                          <Link
                            to="/admin"
                            onClick={() => setShowUserMenu(false)}
                            className="flex items-center space-x-3 px-4 py-2 text-sm text-purple-700 hover:bg-purple-50 transition font-semibold"
                          >
                        ...
                        <button
                          ...
                          className="flex items-center space-x-3 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition font-medium"
                        >
            ...
            <button
              ...
              className="lg:hidden p-2 text-gray-600 hover:text-indigo-600 hover:bg-gray-100 rounded-xl transition"
        ...
        {/* Mobile Menu */}
        {showMobileMenu && (
          <div className="lg:hidden border-t border-gray-200 pb-4 pt-2 space-y-1">
